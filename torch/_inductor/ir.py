...
class Reduction(Loops):
    ...
    def num_splits(
        ...
        def refine_split_num(split_num, reduction_numel_hint, split_hints):
        ...
        if numel_hint == 1:
            ...
            if split > 1 and not ignore_defer_reduction_split and config.defer_reduction_split:
                return ReductionHint.DEFERRED_SPLIT, 1
            return ReductionHint.INNER, refine_split_num(split, reduction_numel_hint, split_hints)
        if (
        ...
        if num_inner > num_outer:
            split_num = inner_reduction_splits(
                reduction_numel_hint, numel_hint
            )
            if split_num > 1 and not ignore_defer_reduction_split and config.defer_reduction_split:
                return ReductionHint.DEFERRED_SPLIT, 1
            return ReductionHint.INNER, refine_split_num(split_num, reduction_numel_hint, split_hints)
        else:
            split_num = outer_reduction_splits(
                reduction_numel_hint, numel_hint
            )
            if split_num > 1 and not ignore_defer_reduction_split and config.defer_reduction_split:
                return ReductionHint.DEFERRED_SPLIT, 1
            return ReductionHint.OUTER, refine_split_num(split_num, reduction_numel_hint, split_hints)
