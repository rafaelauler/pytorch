...
def dump_compiler_graph_state(
    gm, args, compiler_name, *, config_patches=None, accuracy=None, strict=False,
):
    subdir = os.path.join(minifier_dir(), "checkpoints")
    if not os.path.exists(subdir):
        os.makedirs(subdir, exist_ok=True)
...
def export_for_aoti_minifier(gm, tuple_inputs, strict=False, skip_export_error=True) -> Optional[torch.nn.Module]:
...
def repro_minify(options, exported_program, config_patches):
    ...
    def module_fails(gm, flat_example_inputs, check_str=None):
        ...
        gm = export_for_aoti_minifier(gm, tuple_inputs, strict=strict, skip_export_error=skip_export_error)
...
""",
    ...
    parser_minify.add_argument(
        ...
        help="The export mode used in minifier, either dynamo or python",
    )
    parser_minify.add_argument(
        "--skip-export-error",
        type=bool,
