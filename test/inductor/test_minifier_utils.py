...
import torch
from torch._dynamo.repro.aoti import export_for_aoti_minifier, get_module_string, AOTIMinifierError
...
class MinifierUtilsTests(TestCase):
    ...
    def test_non_exportable(self):
        ...
        for strict in [True, False]:
            gm = export_for_aoti_minifier(model, inputs, strict=strict, skip_export_error=True)
            print(gm)
            self.assertTrue(gm is None)

            with self.assertRaises(AOTIMinifierError):
                export_for_aoti_minifier(model, inputs, strict=strict, skip_export_error=False)
